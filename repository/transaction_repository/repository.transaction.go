...
func (repo *TransactionRepositoryImpl) FindAll(ctx context.Context) []entity.Transaction {
	...
	defer helpers.CommitOrRollback(tx)
	err := tx.WithContext(ctx).
		Joins("left join customers on customer_id = trans_customer_id").
		Preload("User").
		Preload("Customer").
		Preload("TransDetail").
...
func (repo *TransactionRepositoryImpl) GetTransactionSummary(ctx context.Context, where map[string]interface{}) map[string]interface{} {
	...
	tx.WithContext(ctx).Raw("SELECT SUM( trans_subtotal ) trans_subtotal,SUM( trans_total ) trans_total, COUNT(*) total_trans, SUM( trans_discount) trans_discount FROM transactions  WHERE trans_user_id = ? "+whereString, where["userId"].(string)).Find(&dataSum)

	/* get product top selling */
	dataProduct := []map[string]interface{}{}
	tx.WithContext(ctx).Raw("SELECT SUM( trans_detail_qty ) trans_detail_qty, SUM( trans_detail_subtotal ) trans_detail_subtotal, CONCAT(product_name,'(',varian_name,')') product_name FROM v_tb_trans_detail JOIN transactions ON trans_id=trans_detail_parent_id WHERE 0 = 0 " + whereString + " GROUP BY product_name ORDER BY SUM(trans_detail_qty) DESC LIMIT 5").Find(&dataProduct)
...
func (repo *TransactionRepositoryImpl) GenInvoice(ctx context.Context) string {
	...
	tx.WithContext(ctx).Table("transactions").Select("IFNULL(trans_invoice,'') trans_invoice").Where("trans_invoice LIKE ?", "%"+date+"%").Order("trans_invoice DESC").Find(invoice)
	inv := ""
