...
func (repo *CustomerRepositoryImpl) FindAll(ctx context.Context, config map[string]interface{}) ([]entity.CustomerResponse, int) {
	customer := []Customer{}
	tx := repo.DB.Begin()
	defer helpers.CommitOrRollback(tx)
	...
	// Count total rows
	var totalRows int64
	countQuery := tx.WithContext(ctx).Model(&Customer{}).
...
func (repo *CustomerRepositoryImpl) UpdateFcm(ctx context.Context, customerId string, fcmToken string) error {
	tx := repo.DB.Begin()
	defer helpers.CommitOrRollback(tx)
	...
	err := tx.WithContext(ctx).
		Model(&Customer{}).
		Where("customer_id = ?", customerId).
		Update("customer_fcm_token", fcmToken).Error
...
func (repo *CustomerRepositoryImpl) GenCustCode(ctx context.Context) string {
	...
	tx.WithContext(ctx).Table("customers").Select("IFNULL(customer_code,'') customer_code").Where("customer_code LIKE ?", "%"+date+"%").Order("customer_code DESC").Find(invoice)
	inv := ""
