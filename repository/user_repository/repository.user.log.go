...
func (repo *UserLogRepositoryImpl) Create(ctx context.Context, userLog entity.UserLog) {
	...
	err := tx.WithContext(ctx).Exec("INSERT INTO user_logs(log_user_id,log_user_user_id,log_user_token,log_user_metadata,log_user_login_date) VALUES(?,?,?,?,?)", helpers.GenUUID(), userLog.LogUserUserId, userLog.LogUserToken, userLog.LogUserMetadata, helpers.CreateDateTime()).Error
