...
func (controller *UseCaseImpl) FindAll(w http.ResponseWriter, r *http.Request) {
	...
	dataResponse, totalRows := controller.CustomerRepository.FindAll(r.Context(), conf)
...
func (controller *UseCaseImpl) DoLogin(w http.ResponseWriter, r *http.Request) {
	...
	if fcmToken, exists := dataLogin["fcm_token"]; exists && fcmToken != nil && fcmToken.(string) != "" {
		err := controller.CustomerRepository.UpdateFcm(r.Context(), user[0].CustomerId, fcmToken.(string))
